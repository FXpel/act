...
if __name__ == "__main__":
    ...
    with open(instance_filename, "r") as f:
        ...
        while True:
            ...
            T.append(int(t))
    if len(A) != n:
        print("wrong number of tasks in instance (the header says {}, but there are {} in the body)".format(n, len(A)), file=sys.stderr)
        sys.exit(1)
    ...
    with open(solution_filename, "r") as f:
        ...
        while True:
            ...
            i += 1
    if i != n:
        print("wrong number of tasks in solution ({} in instance, {} in solution)".format(n, i), file=sys.stderr)
        sys.exit(1)
    ...
    for (t, kind, i) in events:
        if kind == 'start':
            if D[i] < A[i]:
                print("Task {} starts at t={}, which is before its arrival date ({})".format(i, D[i], A[i]), file=sys.stderr)
                sys.exit(1)
            if SIZE[i] > n_idle:
                print("Task {} scheduled at t={} and requires {} machines, but only {} are available".format(i, D[i], SIZE[i], n_idle), file=sys.stderr)
                sys.exit(1)
            n_idle -= SIZE[i]
            R[i] = []
            for j in range(m):
